...
### 3.1 Struct sizes (assumed; must match firmware)

| Struct | Assumed size | Note |
|--------|----------------|------|
| `TRecipeInfo` | **12** bytes | Packed, little-endian. If firmware differs, update app constants. |
| `TRecipeStep` | **32** bytes | Packed; 4× 1-bit bools in one byte. Compiler-dependent. |

*These values are chosen to match the reverse-engineering report and typical packed layout. They have not been confirmed from firmware `sizeof()`; the app MUST use configurable constants so they can be updated.*
...
**Size:** 32 bytes (assumed). **Endianness:** Little-endian for multi-byte fields.
...
| 30 | (reserved/padding) | - | Optional padding to align bitfield |
| 31 | Flags | 4× 1-bit | NeedForTransport, IsTransport, IsProcess, IsStepDone |

**Bitfield byte (offset 31):** Bit order is compiler-dependent. Assumed order (LSB first): bit0 = NeedForTransport, bit1 = IsTransport, bit2 = IsProcess, bit3 = IsStepDone. *If firmware uses different order, update the codec.*
...
**NTAG213 safe recipe page range:** Pages 8–39 (inclusive) = 32 pages = 128 bytes. So max recipe stream length = 128 bytes (header 12 + steps: 116 bytes → 3 steps of 32 bytes + 20 bytes unused, or adjust step count to fit).
...
8. Decode steps from bytes 12 .. 12 + RecipeSteps*32 - 1.
...
- **Exact HEADER_SIZE and STEP_SIZE** on the actual ESP32 build (we use 12 and 32).
...
- **Maximum RecipeSteps** in practice (derived from tag capacity: e.g. (128 - 12) / 32 = 3 steps for 128-byte NTAG213 user region).
